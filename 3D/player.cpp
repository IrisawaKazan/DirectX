...
void UpdatePlayer(void)
{
	...
	////モデルの角度を目標の角度に近づける
	//g_player.rot += (g_player.rotDest - g_player.rot) * 0.075f;
	...
	if (g_player.bDisp == true)
	{
		...
		{//左
			...
			{//左奥に移動
				...
				/*//左
				pCamera->posV.x -= cosf(pCamera->rot.y) * MAX_PMOVE / sqrtf(2.0f);
				pCamera->posV.z += sinf(pCamera->rot.y) * MAX_PMOVE / sqrtf(2.0f);
				...
				//上
				pCamera->posV.x += sinf(pCamera->rot.y) * MAX_PMOVE / sqrtf(2.0f);
				pCamera->posV.z += cosf(pCamera->rot.y) * MAX_PMOVE / sqrtf(2.0f);
			...
			{//左に移動
				...
				/*//左
				pCamera->posV.x -= cosf(pCamera->rot.y) * MAX_PMOVE;
				pCamera->posV.z += sinf(pCamera->rot.y) * MAX_PMOVE;
		...
		{//奥
			...
			/*//上
			pCamera->posV.x += sinf(pCamera->rot.y) * MAX_PMOVE;
			pCamera->posV.z += cosf(pCamera->rot.y) * MAX_PMOVE;
		...
		{//ENTERキーが押された
			//弾の設定
			SetBullet(D3DXVECTOR3(g_player.pos.x, 10.0f, g_player.pos.z),
